...
import com.praveenpayasi.simpleimageloaderdemo.core.ViewHolder
import com.praveenpayasi.simpleimageloaderdemo.core.ViewSize
...
class ImageViewHolder(private val imageView: ImageView) : ViewHolder<ImageView> {
    ...
    override fun optSize(): ViewSize? {
        val width =
            (imageView.layoutParams?.width?.takeIf { it > 0 } ?: imageView.width.takeIf { it > 0 }
            ?: MATCH_PARENT).takeIf { it != WRAP_CONTENT }
                ?: imageView.context.resources.displayMetrics.widthPixels
        val height =
            (imageView.layoutParams?.height?.takeIf { it > 0 } ?: imageView.height.takeIf { it > 0 }
            ?: MATCH_PARENT).takeIf { it != WRAP_CONTENT }
